...
// Adicione após as constantes iniciais, antes do config
...
async function fetchJamendoTracks(limit = 4, genre = "", offset = 0) {
  try {
...
async function initializePlayer() {
  try {
    music_list = await fetchJamendoTracks();
...
initializePlayer();
// Chame initializePlayer() para carregar as músicas do Jamendo
...
// Adicione após as funções de favoritos

let currentPage = 1;
const tracksPerPage = 4;

async function loadMoreTracks() {
  const offset = (currentPage - 1) * tracksPerPage;
...
// Adicione após a função initializePlayer

document.getElementById('genre-select').addEventListener('change', async (e) => {
    const genre = e.target.value;
    currentPage = 1;
    music_list = await fetchJamendoTracks(tracksPerPage, genre);
    track_index = 0;
    loadTrack(track_index);
});
